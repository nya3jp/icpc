...
#define ID(i,j) ((i)+(j)*w)
        ...
        REP(i, w) REP(j, h) {
            ...
            if (IN(i-1, 0, w)) {
                ...
                v.push_back((Edge){ID(i, j), ID(i-1, j),
                            xs[i]-xs[i-1], periods[i-1][j], wetime[i-1][j], !flags[i-1][j]
                            });
            }
            if (IN(i+1, 0, w)) {
                v.push_back((Edge){ID(i, j), ID(i+1, j),
                            xs[i+1]-xs[i], periods[i+1][j], wetime[i+1][j], !flags[i+1][j]
                            });
            }
            if (IN(j-1, 0, h)) {
                v.push_back((Edge){ID(i, j), ID(i, j-1),
                            ys[j]-ys[j-1], periods[i][j-1], wetime[i][j-1], flags[i][j-1]
                            });
            }
            if (IN(j+1, 0, h)) {
                v.push_back((Edge){ID(i, j), ID(i, j+1),
                            ys[j+1]-ys[j], periods[i][j+1], wetime[i][j+1], flags[i][j+1]
                            });
            }
        }
        int sx, sy, tx, ty;
        ...
        while(!q.empty()) {
            ...
            FOR(it, g[e.dest]) if (!visited[it->dest]) {
                ...
                printf("expanding to (%d,%d) time %d : [%d][%d] flag=%d len=%d\n", it->dest%w, it->dest/w, e.weight, it->first, it->period-it->first, it->flag, it->weight);
